...
*/
 ...
 // :return:
 var get_graph_from_plan = function get_graph_from_plan (plan, callback) {
     var parser = N3.Parser();
     var rdfStore = N3.Store();
     parser.parse(plan, function(error, triple, prefixes){
         if (error) {
             callback(generate_error("Turtle Parse"));
         }
         if (triple) {
 ...
 module.exports.get_plan = function get_plan (gp, callback) {
     ...
     try {
         ...
         if (req.statusCode === 200) {
             get_graph_from_plan(req.getBody().toString('utf-8'), function(e){
                 callback(e);
             });
         }
         else {
             callback(generate_error("Planner Response"));
         }
     }
     catch (err) {
         callback(generate_error("Planner Connection"))
     }
 };
